...
enum AVIF_VERSION_MAJOR = 0;
enum AVIF_VERSION_MINOR = 9;
enum AVIF_VERSION_PATCH = 0;
...
void avifRWDataSet(avifRWData* raw, in ubyte* data, size_t len);
...
avifColorPrimaries avifColorPrimariesFind(in float* inPrimaries, in char** outName);
...
void avifImageCopy(avifImage* dstImage, in avifImage* srcImage, uint planes); // deep copy
void avifImageDestroy(avifImage* image);

void avifImageSetProfileICC(avifImage* image, in ubyte* icc, size_t iccSize);

// Warning: If the Exif payload is set and invalid, avifEncoderWrite() may return AVIF_RESULT_INVALID_EXIF_PAYLOAD
void avifImageSetMetadataExif(avifImage* image, in ubyte* exif, size_t exifSize);
void avifImageSetMetadataXMP(avifImage* image, in ubyte* xmp, size_t xmpSize);

void avifImageAllocatePlanes(avifImage* image, uint planes); // Ignores any pre-existing planes
void avifImageFreePlanes(avifImage* image, uint planes);     // Ignores already-freed planes
...
// values.
void avifRGBImageSetDefaults(avifRGBImage* rgb, in avifImage* image);
uint avifRGBImagePixelSize(in avifRGBImage* rgb);
...
avifResult avifImageRGBToYUV(avifImage* image, in avifRGBImage* rgb);
avifResult avifImageYUVToRGB(in avifImage* image, avifRGBImage* rgb);
...
avifCodecChoice avifCodecChoiceFromName(in char* name);
...
struct avifCodecConfigurationBox
{
    ...
    ubyte seqProfile;           // unsigned int (3) seq_profile;
    ubyte seqLevelIdx0;         // unsigned int (5) seq_level_idx_0;
    ubyte seqTier0;             // unsigned int (1) seq_tier_0;
    ubyte highBitdepth;         // unsigned int (1) high_bitdepth;
    ubyte twelveBit;            // unsigned int (1) twelve_bit;
    ubyte monochrome;           // unsigned int (1) monochrome;
    ubyte chromaSubsamplingX;   // unsigned int (1) chroma_subsampling_x;
    ubyte chromaSubsamplingY;   // unsigned int (1) chroma_subsampling_y;
    ubyte chromaSamplePosition; // unsigned int (2) chroma_sample_position;
...
avifIO* avifIOCreateMemoryReader(in ubyte* data, size_t size);
avifIO* avifIOCreateFileReader(in char* filename);
...
avifResult avifDecoderReadMemory(avifDecoder* decoder, avifImage* image, in ubyte* data, size_t size);
avifResult avifDecoderReadFile(avifDecoder* decoder, avifImage* image, in char* filename);
...
avifResult avifDecoderSetIOMemory(avifDecoder* decoder, in ubyte* data, size_t size);
avifResult avifDecoderSetIOFile(avifDecoder* decoder, in char* filename);
...
avifBool avifDecoderIsKeyframe(in avifDecoder* decoder, uint frameIndex);
uint avifDecoderNearestKeyframe(in avifDecoder* decoder, uint frameIndex);
...
avifResult avifDecoderNthImageTiming(in avifDecoder* decoder, uint frameIndex, avifImageTiming* outTiming);
...
//
// This function may be used after a successful call (AVIF_RESULT_OK) to avifDecoderParse().
avifResult avifDecoderNthImageMaxExtent(in avifDecoder* decoder, uint frameIndex, avifExtent* outExtent);
...
avifResult avifEncoderWrite(avifEncoder* encoder, in avifImage* image, avifRWData* output);
...
avifResult avifEncoderAddImage(avifEncoder* encoder, in avifImage* image, ulong durationInTimescales, uint addImageFlags);
avifResult avifEncoderAddImageGrid(avifEncoder* encoder,
                                   uint gridCols,
                                   uint gridRows,
                                   in avifImage** cellImages,
                                   uint addImageFlags);
...
void avifEncoderSetCodecSpecificOption(avifEncoder* encoder, in char* key, in char* value);

// Helpers
avifBool avifImageUsesU16(in avifImage* image);
...
avifBool avifPeekCompatibleFileType(in avifROData* input);
